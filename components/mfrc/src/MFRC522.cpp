...
MFRC522::MFRC522(MFRC522_BUS_DEVICE * dev) {
	this->_dev  = dev;
};


void MFRC522::PCD_WriteRegister(MFRC522::PCD_Register reg, uint8_t value) 
{
    _dev->PCD_WriteRegister(reg,value);
};

void MFRC522::PCD_WriteRegister(MFRC522::PCD_Register reg, uint8_t count, uint8_t *values) 
{
    _dev->PCD_WriteRegister(reg,count,values);
};

uint8_t MFRC522::PCD_ReadRegister(MFRC522::PCD_Register reg) 
{
    return _dev->PCD_ReadRegister(reg);
};
void MFRC522::PCD_ReadRegister(MFRC522::PCD_Register reg, uint8_t count, uint8_t *values, uint8_t rxAlign) 
{
...
{ 
	...
	_dev->PCD_WriteRegister(reg, tmp | mask); 
} 
...
int MFRC522::PCD_CalculateCRC(uint8_t *data, uint8_t length,uint8_t *result) {
	...
	for (uint16_t i = 5000; i > 0; i--) {
		uint8_t n = _dev->PCD_ReadRegister(DivIrqReg);
...
void MFRC522::PCD_AntennaOn() {
	uint8_t value = _dev->PCD_ReadRegister(TxControlReg);
	if ((value & 0x03) != 0x03) {
...
MFRC522::PCD_PerformSelfTest() {
	...
	uint8_t version = _dev->PCD_ReadRegister(VersionReg);
...
{/
	uint8_t val = _dev->PCD_ReadRegister(CommandReg); // Read state of the command register 
...
int MFRC522::PCD_TransceiveData(	uint8_t *sendData,		///< Pointer to the data to transfer to the FIFO.
...
int MFRC522::PCD_CommunicateWithPICC(uint8_t command,///< The command to execute. One of the PCD_Command enums.
									 ...
									 ) 
{
	...
	for (uint16_t i = 2000; ; --i) {
		uint8_t n = _dev->PCD_ReadRegister(ComIrqReg);	// ComIrqReg[7..0] bits are: Set1 TxIRq RxIRq IdleIRq HiAlertIRq LoAlertIRq ErrIRq TimerIRq
		if (n & waitIRq) {					// One of the interrupts that signal success has been set.
			break;
		}
		if (n & 0x01 || i == 0) {						// Timer interrupt - nothing received in 25ms
			return STATUS_TIMEOUT;
		}
	...
	uint8_t errorRegValue = _dev->PCD_ReadRegister(ErrorReg); // ErrorReg[7..0] bits are: WrErr TempErr reserved BufferOvfl CollErr CRCErr ParityErr ProtocolErr
	...
	if (backData && backLen) {
		uint8_t n = _dev->PCD_ReadRegister(FIFOLevelReg);	// Number of bytes in the FIFO
		if (n > *backLen) {
			return STATUS_NO_ROOM;
		}
		*backLen = n;											// Number of bytes returned
		_dev->PCD_ReadRegister(FIFODataReg, n, backData, rxAlign);	// Get received data from FIFO
		_validBits = _dev->PCD_ReadRegister(ControlReg) & 0x07;		// RxLastBits[2:0] indicates the number of valid bits in the last received byte. If this value is 000b, the whole uint8_t is valid.
		if (validBits) {
...
int MFRC522::PICC_RequestA(	uint8_t *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
...
int MFRC522::PICC_WakeupA(	uint8_t *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
							uint8_t *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
										) {
	return PICC_REQA_or_WUPA(PICC_CMD_WUPA, bufferATQA, bufferSize);
} 

int MFRC522::PICC_REQA_or_WUPA(	uint8_t command, ///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
...
int MFRC522::PICC_Select(Uid *uid, uint8_t validBits) 
{
	bool uidComplete;
	bool selectDone;
	bool useCascadeTag;
...
int MFRC522::PCD_Authenticate(uint8_t command, ///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
...
void MFRC522::PCD_StopCrypto1() {
	PCD_ClearRegisterBitMask(Status2Reg, 0x08); // Status2Reg[7..0] bits are: TempSensClear I2CForceHS reserved reserved 
} 

int MFRC522::MIFARE_Read(uint8_t blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
...
int MFRC522::MIFARE_Write(	uint8_t blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
 ...
 */
int MFRC522::MIFARE_Ultralight_Write(	uint8_t page, 		///< The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::MIFARE_Decrement(	uint8_t blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::MIFARE_Increment(	uint8_t blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::MIFARE_Restore(	uint8_t blockAddr ///< The block (0-0xff) number.
											) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::MIFARE_TwoStepHelper(	uint8_t command,	///< The command to use
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::MIFARE_Transfer(	uint8_t blockAddr ///< The block (0-0xff) number.
											) {
	int result;
	uint8_t cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
  */
int MFRC522::MIFARE_GetValue(uint8_t blockAddr, int32_t *value) {
	int status;
	uint8_t buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::MIFARE_SetValue(uint8_t blockAddr, int32_t value) {
	uint8_t buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::PCD_NTAG216_AUTH(byte* passWord, uint8_t pACK[]) //Authenticate with 32bit password
{
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
int MFRC522::PCD_MIFARE_Transceive(	uint8_t *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
 ...
 */
const __FlashStringHelper *MFRC522::GetStatusCodeName(int code	///< One of the StatusCode enums.
										) {
	switch (code) {
...
void MFRC522::PCD_DumpVersionToSerial() {
	...
	uint8_t v = _dev->PCD_ReadRegister(VersionReg);
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors) {
	...
	uint8_t received;
	int status = PCD_TransceiveData(&cmd, (byte)1, response, &received, &validBits, (byte)0, false); // 40
	if(status != STATUS_OK) {
		if(logErrors) {
...
bool MFRC522::MIFARE_SetUid(uint8_t *newUid, uint8_t uidSize, bool logErrors) {
	...
	int status = PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key, &uid);
	if (status != STATUS_OK) {
...
bool MFRC522::MIFARE_UnbrickUidSector(bool logErrors) {
	...
	// Write modified block 0 back to card
	int status = MIFARE_Write((byte)0, block0_buffer, (byte)16);
	if (status != STATUS_OK) {
		if (logErrors) {
			Serial.print(F("MIFARE_Write() failed: "));
			Serial.println(GetStatusCodeName(status));
