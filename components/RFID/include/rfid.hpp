...
public:
	...
	int PCD_ClrRegisterBits(uint8_t reg, uint8_t mask);

	int PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result);

	int PCD_TransceiveData (uint8_t *sendData,
							uint8_t sendLen,
							uint8_t *backData,
							uint8_t *backLen,
	...
	int PCD_CommunicateWithPICC(uint8_t command,
								uint8_t waitIRq,
								uint8_t *sendData,
								uint8_t sendLen,
	...
	int PICC_RequestA(uint8_t *bufferATQA, uint8_t *bufferSize);

	int PICC_WakeupA(uint8_t *bufferATQA, uint8_t *bufferSize);
...
private:
	...
	int MIFARE_TwoStepHelper(uint8_t command, uint8_t blockAddr, uint32_t data);
...
enum PICC_Type {
	...
	static constexpr size_t MAX_TYPENAMES = 10;
	static const constexpr char* const _ErrorMessage[MAX_TYPENAMES] =
	{
		"Unknown error",
		"Success",
	...
	int write_byte_reg(const uint8_t b);
};
