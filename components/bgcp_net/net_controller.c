...
// bool check_bgcp_protocol(char *data, uint16_t size)
// {
...
// {
//     char rx_buffer[UDP_RECV_BUFFER_SIZE];
...
//         device_wait_bits(AP_CLIENT, portMAX_DELAY);
//         if (udp_socket == -1) {
...
//                     device_wait_bits(PARSE, portMAX_DELAY);
//                     device_set_bits(PARSE);
//                     parse(rx_buffer, len);
//                     // Send response if needed
//                     if (get_req_queue_size()){
//                         if (sendto(udp_socket, get_req_queue_data(), get_req_queue_size(), 0, (struct sockaddr *)&source_addr, sizeof(source_addr)) < 0) {
//                             ESP_LOGE(TAG, "Failed to send response: errno %d", errno);
//                         }
//                     }
//                     device_clear_bits(PARSE);
//                 }
//             }
//         }

//         if (!(device_get_bits()&AP_CLIENT)){
